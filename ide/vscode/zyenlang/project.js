...
function loadProject(startPath) {
  ...
  return parseManifest(fs.readFileSync(manifestPath, 'utf8'), root);
}

function importFile(importPath, currentFile, project) {
  const parts = String(importPath).split('::');
  const namespace = parts.shift();
  if (!namespace || !parts.length || namespace === 'std') return undefined;
  let root;
  ...
  if (!root) return undefined;
  const candidate = path.resolve(root, 'src', ...parts) + '.zy';
  const sourceRoot = path.resolve(root, 'src') + path.sep;
  return candidate.startsWith(sourceRoot) ? candidate : undefined;
}

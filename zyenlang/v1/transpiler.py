...
def resolve_std_import_path(import_name: str) -> Path:
    ...
    v0.1.17 supports both source-tree execution and installed-module execution:
      ...
      - installed package: zyenlang/v1/std/math.zy
    """
    name = import_name.strip().replace("\\", "/")
    roots = [
        Path(__file__).resolve().parent,       # installed package: zyenlang/v1/std
        Path(__file__).resolve().parents[2],  # source tree: project/std
    ]
